    ...
    return {
        ...
        'newOrModify': function (selector, styleObjOrCssName, cssValue) {
            if (typeof styleObjOrCssName === 'string') {
                const currentStyle = draftStyle[selector] || {};
                currentStyle[styleObjOrCssName] = cssValue;
                draftStyle[selector] = currentStyle;
        ...
        'discard': function () {
            if (this.hasChange()) {
                ...
                if (discardFuncs.length > 0) {
                    discardFuncs.forEach(func => {
                        func();
                    });
                }
            }
        ...
        //     discardFuncs = [];
        // },
        'remove': function (_selector, cssNameOrArray) {
            if (!!cssNameOrArray) {
        ...
        'registerDiscard': function (func) {
            discardFuncs.push(func);
        }
    };
});